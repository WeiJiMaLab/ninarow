...
class DataLoader: 
    '''
    DataLoader class to load data from a DataFrame and yield Board and Move objects.
    '''
    ...
    def __call__(self):
        ...
        for row in df.itertuples(): 
            board = Board(row.black, row.white)
            move = Move(row.move, row.color)

            assert board.active_player == move.color, f"Move color does not match active player on board: {board.to_string()}"
            yield board, move.move_index


class Board:
    '''
    Board class to represent the state of the board.
    ''' 
    def __init__(self, black, white): 
        self.black = black
        self.white = white
        self.board = fourbynine_board(
                        fourbynine_pattern(self.black), 
                        fourbynine_pattern(self.white)
                    )
        
        self.active_player = self.board.active_player()
    
    def __repr__(self): 
        return f"Board(black = {int(self.board.get_pieces(Player_Player1).to_string(), 2)}, white = {int(self.board.get_pieces(Player_Player2).to_string(), 2)})"
...
class Move: 
    '''
    Move class to represent a move made by a player.
    '''
    def __init__(self, move, color):
        '''
        Parameters:
            move (int): The move made by the player, represented as an int that is decomposable into a binary board.
            color (str): The color of the player making the move ('white' or 'black').
        '''
        color = color.lower()
        assert color in ['white', 'black'], f"Invalid color given: {color}. Must be 'white' or 'black'."
        assert bin(move).count('1') == 1, f"Invalid move given: {move} does not represent a valid move (must have exactly one space occupied)."

        # color is 0 if black and 1 if white
        self.color = color == 'white'

        # get the bit index where move is 1
        move_index = int(move).bit_length() - 1
        self.move_index = move_index
        self.move = fourbynine_move(self.move_index, 0.0, self.color)

    def __repr__(self): 
        return f"Move(index = {self.move.board_position}, color = {"white" if self.color else "black"})"
